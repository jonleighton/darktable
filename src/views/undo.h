...
void dt_undo_record(dt_undo_t *data, dt_view_t *view, dt_undo_type_t type, dt_undo_data_t *item, void (*undo) (dt_view_t *view, dt_undo_type_t type, dt_undo_data_t *item));

//  undo an element which correspond to filter. filter here is expected to be
//  a set of dt_undo_type_t.
void dt_undo_do_undo(dt_undo_t *data, uint32_t filter);

//  redo a previously undone action, does nothing if the redo list is empty
void dt_undo_do_redo(dt_undo_t *data, uint32_t filter);

//  removes all items which correspond to filter in the undo/redo lists
void dt_undo_clear(dt_undo_t *data, uint32_t filter);
